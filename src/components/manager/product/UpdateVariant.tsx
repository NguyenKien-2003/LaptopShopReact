...
const UpdateVariant: React.FC = () => {
  ...
  const handleRemoveImage = async (index: number) => {
    ...
    try {
      const response = await deleteOtherImage(
        Number(id),
        variant.imageOthers[index]
      );
      if (response) {
        fetchVariant();
      } else {
    ...
    <Card sx={{ maxWidth: 800, margin: "auto", padding: 3, marginTop: 5 }}>
      ...
      <Box display="flex" flexDirection="column" gap={2} marginTop={3}>
        ...
        <TextField
          ...
          onChange={(e) =>
            setVariant({ ...variant, stockQuantity: Number(e.target.value) })
          }
          fullWidth
        />
        <TextField
      ...
      <Box sx={{ marginTop: 3, display: "flex", justifyContent: "end" }}>
        ...
        <Button
          ...
          disabled={
            ...
            variant.price < 0 ||
            (currentColor === selectedColor && currentSize === selectedSize)
          }
        >
          Lưu thay đổi
        </Button>
