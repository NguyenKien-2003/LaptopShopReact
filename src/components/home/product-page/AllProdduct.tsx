...
const AllProdduct: React.FC = () => {
    ...
    const getAllProductVariant = async () => {
        try {
            setIsLoading(true);
            const { minPrice, maxPrice, brandIds, categoryIds} = filters;
            const response = await filterProductVariantDefaults(minPrice, maxPrice, brandIds, categoryIds ,0, size, '', '');
        ...
        <Box sx={{ display: 'flex', backgroundColor: '#f9fafb', width: '100%' }}>
            ...
            <Container maxWidth="xl" sx={{ width: '78%', py: 4, px: { xs: 2, md: 4 } }}>
                <Box className="flex items-center justify-between mb-6">
                    ...
                    >
                        <Typography variant="h5" fontWeight={"bold"} component="h1" className=" text-gray-800 uppercase">
                        ...
                        {products.length > 0 ? products.map((product) => (
                            ...
                            >
                                ...
                                <Box className="p-4">
                                    <Typography 
                                        fontWeight={"bold"} 
                                        className="font-semibold text-gray-800 line-clamp-2 h-14 mb-2 text-left"
                                        sx={{ minHeight: '3.5rem' }}
                                    >
                                    ...
                                    <div className="mt-2">
                                        {product.discountRate > 0 ? (
                                            <div className="flex flex-col">
                                                ...
                                                <Typography variant="subtitle1" className="text-red-600 font-bold text-end">
                                                    {product.priceAfterDiscount.toLocaleString()} VNĐ
                                                </Typography>
                                            </div>
                                        ) : (
                                            <Typography variant="subtitle1" className="text-gray-700 font-semibold text-end">
                                                {product.price.toLocaleString()} VNĐ
                                            </Typography>
                                        )}
                                    </div>
